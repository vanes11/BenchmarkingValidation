...
class InteractQuickFill:
    ...
    def GetClassC(self):
        ...
        self.ClasseC = {}
        chemin = currentDirectory + '/classeC.csv'
    ...
    def GetGoodExamples(self):
        ...
        decoupe = []
        DicoToreturn = {}
        chemin = currentDirectory + '/dataResultQuickFill.txt'
        with open(chemin,"r", newline=None) as f:
            for line in f:
                ...
                if len(decoupe)==1:
                    ...
                    DicoToreturn[json.dumps(sigma)] = sortie
                    s = s.union(set([x]))
                    sigma = {}
    ...
    def GetTestExemaples(self):
        result = {}
        sigma = {}
        currentDirectory = os.getcwd()
        chemin = currentDirectory + '/'+ 'dataQuickFill.txt'
        with open(chemin,"r", newline=None) as f:
            for line in f:
                line = line.replace("\n","")
                if line != "":
...
######################### Eriture de la fonction des programmes
    ...
    def ExecuteBaseCaseSubstr2(self,elt,valeur):
        ...
        if Test != None and len(Test) > 0 and len(Test)>=(c-1):
            return Test[c-1] 
    ...
    def FilterProgrammesExecution(self,casekey , Sorties , Entrers, ListOfProgrammes):
        ...
        for elt in Entrers:
            ...
            if compteur < casekey:
                for prog in Result:
                    RepValue = self.ExecuteFonction(prog,Entrers[elt])
